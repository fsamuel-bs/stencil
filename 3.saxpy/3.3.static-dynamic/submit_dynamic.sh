...
CODE_DIR=./code/dynamic-Threads-${NUM_THREADS}-Mode-${AFFINITY}
BIN_DIR=./bin/dynamic-Threads-${NUM_THREADS}-Mode-${AFFINITY}
ASS_DIR=./assembly/dynamic-Threads-${NUM_THREADS}-Mode-${AFFINITY}
REPORT_DIR=./report/dynamic-Threads-${NUM_THREADS}-Mode-${AFFINITY}
OUTPUT_DIR=./output/dynamic-Threads-${NUM_THREADS}-Mode-${AFFINITY}
...
do
    ...
    sed -e s/\{1\}/$SAXPY_SIZE/g \
        -e s/\{2\}/$LOOP_COUNT/g \
        -e s/\{3\}/$NUM_THREADS/g \
        saxpy_dynamic.c > $CODE_DIR/saxpy_$i.c
done
